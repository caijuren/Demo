...
export default function DashboardCard({ metric, index }: DashboardCardProps) {
  ...
  return (
    <div
      className="card-glow group relative flex-1 min-w-[180px] rounded-lg border border-[#e2e8f0] bg-white p-5 transition-all duration-300 hover:border-[#00b4d8]/30 hover:shadow-lg hover:shadow-[#00b4d8]/5"
    ...
    >
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="flex items-center gap-1 px-2 py-0.5 rounded-md bg-[#f1f5f9]">
      ...
      <div className="space-y-1">
        <span className="block font-mono text-2xl font-bold tracking-tight" style={{ color: metric.color }}>
          <span className="animate-number">{formattedValue}</span>
        </span>
        <span className="block text-sm text-[#64748b] font-medium">
          {metric.label}
        </span>
      </div>
    </div>
